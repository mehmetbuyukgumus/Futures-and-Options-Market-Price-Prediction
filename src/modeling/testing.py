...
def prep_data_of_model(comp_name_1, comp_name_2):
    data = historical_prices(comp_name_1, comp_name_2, "2020-07-08", dt.datetime.now().date())
...
def grid_seach_cv(X, y, X_train, X_test, y_train, y_test, model, name, regressor, param):
    ...
    return result_new, model_final

def train_model(comp_name_1, comp_name_2):
        data = prep_data_of_model(f"{comp_name_1}.IS", f"{comp_name_2}.IS")
